...
hljs.LANGUAGES.bash = function( a ) {
	...
	return {
		l: /-?[a-z]+/,
		k: {
			keyword: "curl if then else elif fi for break continue while in do done exit return set declare case esac export exec",
			literal: "true false",
			built_in: "printf echo read pwd pushd popd dirs let eval unset typeset readonly getopts source shopt caller type bind help sudo",
			operator: "-ne -eq -lt -gt -f -d -e -s -l -a -G"
		},
		c: [ {
				cN: "shebang",
				b: /^#![^\n]+sh\s*$/,
